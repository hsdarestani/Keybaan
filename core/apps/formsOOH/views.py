    ...
    if 'AddCustomer' in request.POST:
        ...
        if (form4.is_valid()):
            ...
            try:
                ...
                cuscheck = 0
            if (cuscheck):
                messages.warning(request, "این نام مشتری قبلا ثبت شده است")
                obj.save()            
            else:
                obj.EntryAgent = user
                obj.Company_id = company
                messages.success(request, " مشتری "+str(obj.CustomerName)+" با موفقیت ثبت شد ")
                obj.save()
    ...
    if 'ContractNumber' in request.POST:
        ...
        if all([form1.is_valid(),formset2.is_valid(),formset.is_valid()]):
            ...
            if (conNcheck):
                ...
                obj.Company_id = company
                obj.save()
                messages.info(request, "قرارداد "+str(obj.ContractNumber)+"با موفقیت ثبت شد")
                ...
                for form2 in formset2:
                    ...
                    if (SameBoardscheck):
                        ...
                        obj2.save()
                        messages.info(request, "تابلوی "+str(obj2.BoardID)+"با موفقیت ثبت شد")
                ...
                for form in formset:
                     ...
                     obj3.save()
                     messages.info(request, "قسط "+str(obj3.PaymentDateJalali)+"با موفقیت ثبت شد")
...
@login_required
@OOH_user
def ocontract(request):
    form1 = OCustomersForm()
    form2 = OContractsForm()
    form3 = OContractorForm()
    form2set = formset_factory(OContractDetailsPerBoardForm)
    form3set = formset_factory(OOperationForm)
    # form3 = modelformset_factory(Installment,form=InstallmentForm)
    # qs = objb.Installment_set.all()
    # formset3 = form3(request.POST)
    formset2 = form2set(request.POST or None,prefix='formset2')
    formset = form3set(request.POST or None,prefix='formset3')
    mamadBoards = OBoards.objects.filter(Company_id=request.user.profile.company_id)

    # form1.fields["CustomerID"].queryset = OCustomers.objects.filter(Company_id=request.user.profile.company_id)
    # mamadBoards = Boards.objects.filter(Company_id=request.user.profile.company_id)
    #
    if 'AddCustomer' in request.POST:
        user = request.user.profile
        company = request.user.profile.company_id
        form1 = OCustomersForm(request.POST)
        if (form1.is_valid()):
            obj = form1.save(commit=False)
            try:
                get_object_or_404(OCustomers,CustomerName = obj.CustomerName)
                cuscheck = 1
            except:
                cuscheck = 0
            if (cuscheck):
                ...
                obj.Company_id = company
                obj.save()
            return redirect('formsOOH:ocontract')
    if 'ContractNumber' in request.POST:
        user = request.user.profile
        company = request.user.profile.company_id
        form2 = OContractsForm(request.POST)
    #   form2 = ContractDetailsPerBoardForm(request.POST)
    #     form3 = InstallmentForm(request.POST)
        if all([form2.is_valid(),formset2.is_valid(),formset.is_valid()]):
            obj = form2.save(commit=False)
            obj.ContractConfirmDate = jalali.Persian(str(obj.ContractConfirmDateJalali)).gregorian_string()
            try:
                conNcheck = get_object_or_404(OContracts,ContractNumber = obj.ContractNumber)
            except:
                conNcheck = 0
            if (conNcheck):
                 messages.info(request, "این شماره قرارداد قبلا ثبت شده است")
    ...
    return render(request, 'apps/formsOOH/templates/formsOOH/ocontract.html', context)


@login_required
@OOH_user
def ocontractorpayment(request):
    ...
    return render(request, 'apps/formsOOH/templates/formsOOH/ocontractorpayment.html', context)
